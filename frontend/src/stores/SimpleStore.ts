 ...
 */
@injectable()
export default abstract class SimpleStore<T> implements ValueMonitor<T> {
  /**
   * Contains the state.
   * Returns the current state immediately after subscribing.
   * @protected
   */
  protected storeSubject: BehaviorSubject<T> | null = null;
  ...
  private init(): BehaviorSubject<T> {
    if (this.storeSubject === null) {
      this.storeSubject = new BehaviorSubject<T>(this.getInitialValue());
    }
